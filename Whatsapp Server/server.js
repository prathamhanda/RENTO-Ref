const { default: makeWASocket, DisconnectReason, useMultiFileAuthState, Browsers, downloadMediaMessage, Mimetype } = require('@whiskeysockets/baileys');
const fs = require('fs');
...
const connectToWhatsApp = async () => {
  const { state, saveCreds } = await useMultiFileAuthState('auth_info_goyalinfocom3');
...
const sendWhatsAppMessage = async (phone, message) => {
    ...
    await sockClient.sendMessage(jid, { text: message });
};
        ...
        for (const user of users) {
          ...
          for (const message of user.messages) {
            ...
            if (message.type === 'text') {
                ...
                try {
                    const response = await axios.get(https:${message.url}, { responseType: 'arraybuffer' });
          ...
          socket.emit('replyFromSocket', { message: Messages sent to ${username} });
        }
